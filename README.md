...
It's common to use paraphrase enabled SSH keys to add an extra layer of protection to your SSH keys. You can use those keys with `deb-mup` too. In order to do that, you need to use a `ssh-agent`.
...
* After that simply invoke `deb-mup` commands and they'll just work
