...
exports.setup = function(config) {
  ...
  taskList.copy('Copying ' + config.appName + ' init.d script', {
    src: path.resolve(TEMPLATES_DIR, 'meteor'),
    dest: '/tmp/' + config.appName,
    vars: {
      appName: config.appName
    }
  });

  taskList.execute('Moving ' + config.appName + ' init.d script', {
    command: '(sudo mv /tmp/' + config.appName + ' /etc/init.d/' + config.appName + ')'
  });

  taskList.execute('Configuring ' + config.appName + ' init.d script', {
    command: '(sudo chmod +x /etc/init.d/' + config.appName + ') && (sudo update-rc.d ' + config.appName + ' defaults)'
  });
...
exports.stop = function(appName) {
  ...
  taskList.execute('Stopping app', {
    command: '(sudo service ' + appName + ' stop)'
  });
...
function configureStud(taskList, pemFilePath, port) {
  ...
  taskList.copy('Copying Stud init.d script', {
    src: path.resolve(TEMPLATES_DIR, 'stud'),
    dest: '/tmp/stud'
  });

  taskList.execute('Moving Stud init.d script', {
    command: '(sudo mv /tmp/stud /etc/init.d/stud)'
  });

  taskList.execute('Configuring Stud init.d script', {
    command: '(sudo chmod +x /etc/init.d/stud) && (sudo update-rc.d stud defaults)'
  });
  ...
  // restart stud
  taskList.execute('Restarting Stud', {
    command: '(sudo service stud stop || :) && (sudo service stud stop || :)'
  });
}
